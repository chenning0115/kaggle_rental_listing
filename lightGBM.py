...
for train, test in k_fold.split(train_X, y=train_y):
    x_for_train = train_X.iloc[train].values
    y_for_train = train_y[train]
    x_for_test = train_X.iloc[test].values
    y_for_test = train_y[test]
    lgb_train = lgb.Dataset(x_for_train, y_for_train)
    lgb_eval = lgb.Dataset(x_for_test, y_for_test, reference=lgb_train)
    gbm = lgb.train(params,lgb_train,
        num_boost_round=2000,
        valid_sets=lgb_eval,
        early_stopping_rounds=30)
    preds = gbm.predict(test_X.values,num_iteration=gbm.best_iteration)
    sub = pd.DataFrame(data = {'listing_id': test_X['listing_id'].ravel()})
    sub['high'] = preds[:, 0]
    sub['medium'] = preds[:, 1]
    sub['low'] = preds[:, 2]
    sub.to_csv("light_submission.csv", index = False, header = True)
    break
